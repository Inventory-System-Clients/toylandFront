...
import {
  salvarUltimaMovimentacaoWhatsapp,
  obterUltimaMovimentacaoWhatsapp,
} from "../utils/ultimaMovimentacaoWhatsapp";
...
export function Movimentacoes() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Monta a mensagem com os dados ainda no formulário, salva como
      // "última movimentação" (sobrescrevendo a anterior) e tenta levar a
      // pessoa direto para o WhatsApp. Precisa ser feito antes de resetar
      // o formData/fotoContadores logo abaixo.
      const mensagemWhatsapp = montarMensagemWhatsappFormulario();
      salvarESendWhatsapp(mensagemWhatsapp, fotoContadores);
  ...
  // --- WHATSAPP ---
  // Monta o texto da mensagem a partir do formulário de movimentação ainda
  // preenchido (precisa ser chamada antes do formData ser resetado).
  const montarMensagemWhatsappFormulario = () => {
  ...
  // Tenta levar a pessoa direto para o WhatsApp: compartilha texto + foto
  // (quando o navegador suporta) ou abre o wa.me só com o texto. Se nada
  // disso for possível (popup bloqueado, sem suporte etc.), a mensagem já
  // ficou salva no navegador e pode ser reenviada pelo botão "Enviar última
  // movimentação".
  const tentarEnviarWhatsapp = async (mensagem, foto) => {
    if (
      foto &&
      navigator.canShare &&
      navigator.share &&
  ...
  // Salva a mensagem/foto como "última movimentação" (sobrescrevendo a
  // anterior) e tenta enviar direto para o WhatsApp.
  const salvarESendWhatsapp = async (mensagem, foto) => {
    try {
      await salvarUltimaMovimentacaoWhatsapp({ mensagem, foto });
    } catch (err) {
      console.error("Erro ao salvar última movimentação para WhatsApp:", err);
    }
    await tentarEnviarWhatsapp(mensagem, foto);
  };

  const reenviarUltimaMovimentacaoWhatsapp = async () => {
    try {
      const salvo = await obterUltimaMovimentacaoWhatsapp();
      if (!salvo?.mensagem) {
        setError("Nenhuma movimentação recente foi encontrada para reenviar.");
        return;
      }
      await tentarEnviarWhatsapp(salvo.mensagem, salvo.foto);
    } catch (err) {
      console.error("Erro ao reenviar última movimentação:", err);
      setError("Não foi possível reenviar a última movimentação.");
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              type="button"
              className="px-6 py-3 bg-green-600 text-white rounded hover:bg-green-700 font-bold shadow text-base flex items-center gap-2"
              onClick={reenviarUltimaMovimentacaoWhatsapp}
              title="Reenvia a última movimentação salva no navegador para o WhatsApp"
            >
